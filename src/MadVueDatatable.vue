<template>
  <div :class="classes">
    <table
      ...
      ref="table"
      class="stripe hover"
      style="width:100%; padding-top: 1em;  padding-bottom: 1em;margin:0px;"
      :class="className"
      cellpadding="0"
    />
  </div>
          ...
          for (let ii in that.dtfields) {
            if (numbering == i) {
              ...
              if (field.hasOwnProperty('searchable')) {
                if (field.searchable) {
                  $(this).html(
                    '<input type="text" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="' +
                      title +
                      '" value="' +
